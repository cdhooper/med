...
#define	MX	36			/* special register (MMX reg %mm0-7) */
#define	EMX	37			/* special register (MMX reg %mm0-7) */
#define	XMM	38			/* special register (FP reg %xmm0-7) */
#define	EXMM	39			/* special register (FP reg %xmm0-7) */
...
#ifdef PORT_MED
int
mem_disassemble_get_value(uint64_t addr, size_t len, int sign,
                          unsigned int *result)
{
    switch (len) {
        case 1:
...
void
mem_disassemble_print_sym(int32_t off, int strategy)
{
    if (strategy == 0) {
        /* Check for signed (local) offset */
        if ((((unsigned) off >> 31) != 0) && ((0 - off) < 65536)) {
            off = 0 - off;
            printf("$-%x", off);
        } else {
            printf("$%x", off);
...
/*aa*/	{ "rsm",   FALSE, NONE,  0,	      0 },  // IA32 Return from SMM
/*ab*/	{ "bts",   TRUE,  LONG,  op2(R,E),    0 },
/*ac*/	{ "shrd",  TRUE,  LONG,  op3(Ib,E,R), 0 },
/*ad*/	{ "shrd",  TRUE,  LONG,  op3(CL,E,R), 0 },
/*ae*/	{ "fxsave",TRUE,  LONG,  0,	      0 },  // IA32 Save FPU, MMX, SSE
...
#define db_printsym(off, strat, func) \
        mem_disassemble_print_sym((int32_t) off, strat)

/*
 * Replacement for old '%r' kprintf format.
 */
void
db_format_radix(char *buf, size_t bufsiz, uint32_t val, int altflag)
{
    if (altflag)
        sprintf(buf, "%x", val);
    else
...
int
mem_get_value(uint64_t loc, size_t size, int is_signed, unsigned int *result)
{
    if (mem_disassemble_get_value(loc, size, is_signed, result)) {
        printf("Fetch from ");
        db_printsym(loc, DB_STGY_XTRN, NULL);
        printf(" failed\n");
        return (1);
    }
    return (0);
...
#define	get_value_inc(result, loc, size, is_signed) \
	{ \
            if (mem_get_value(loc, size, is_signed, &result)) { \
                return (1);                                                 \
            }                                                               \
            (loc) += (size);                                                \
	}
...
{
	unsigned int	mod, rm, sib, index, disp;
	...
	struct i_addr	*addrp)
{
	...
	if (seg)
        ...
        if (size == BYTE)
            ...
            off = (short) addrp->disp;
        else
            off = (int) addrp->disp;

	db_printsym((db_addr_t) off, DB_STGY_ANY, db_printf);
...
#define SPEC_660F6F_MOVDQA 0  // 66 0f 6f
#define SPEC_660F7F_MOVDQA 1  // 66 0f 7f
#define SPEC_660FD6_MOVQ   2  // 66 0f d6
#define SPEC_F30F6F_MOVDQU 3  // f3 0f 6f
#define SPEC_F30F7E_MOVQ   4  // f3 0f 7e
#define SPEC_F30F7F_MOVDQU 5  // f3 0f 7f
#define SPEC_F390_PAUSE    6  // f3 90

static const struct inst db_inst_special[] = {
    { "movdqa", TRUE,  NONE,  op2(EXMM, XMM), 0 },  // SPEC_660F6F_MOVDQA
    { "movdqa", TRUE,  NONE,  op2(XMM, EXMM), 0 },  // SPEC_660F7F_MOVDQA
    { "movq",   TRUE,  NONE,  op2(XMM, EXMM), 0 },  // SPEC_660FD6_MOVDQA
    { "movdqu", TRUE,  NONE,  op2(EXMM, XMM), 0 },  // SPEC_F30F6F_MOVDQU
    { "movq",   TRUE,  NONE,  op2(EXMM, XMM), 0 },  // SPEC_F30F7E_MOVQ
    { "movdqu", TRUE,  NONE,  op2(XMM, EXMM), 0 },  // SPEC_F30F7F_MOVDQU
    { "pause",  FALSE, NONE,  0,   0 },             // SPEC_F390_PAUSE
};

/*
 * get_66_escape
 * -------------
 * 0x66 word instruction is also used to prefix other instructions
 */
const struct inst *
get_66_escape(db_addr_t *loc)
{
    const struct inst *ip   = NULL;
    unsigned int       inst;

    if (mem_get_value((*loc), 1, FALSE, &inst))
        return (NULL);

    switch (inst) {
        case 0x0f:  // Instruction prefix
            if (mem_get_value((*loc) + 1, 1, FALSE, &inst))
                return (NULL);
            switch (inst) {
                case 0x6f:  // movdqa
                    ip = &db_inst_special[SPEC_660F6F_MOVDQA];
                    break;
                case 0x7f:  // movdqa
                    ip = &db_inst_special[SPEC_660F7F_MOVDQA];
                    break;
                case 0xd6:  // movq
                    ip = &db_inst_special[SPEC_660FD6_MOVQ];
                    break;
            }
            if (ip != NULL)
                *loc += 2;
 ...
 * get_f3_escape
 * -------------
 * 0xf3 repe instruction is also used to prefix other instructions
 */
const struct inst *
get_f3_escape(db_addr_t *loc)
{
    const struct inst *ip   = NULL;
    unsigned int       inst;
    ...
    switch (inst) {
        case 0x90:  // Pause
            *loc += 1;
            ip = &db_inst_special[SPEC_F390_PAUSE];
            break;
        case 0x0f:  // Instruction prefix
            if (mem_get_value((*loc) + 1, 1, FALSE, &inst))
                return (NULL);
            switch (inst) {
                case 0x6f:  // movdqu
                    ip = &db_inst_special[SPEC_F30F6F_MOVDQU];
                    break;
                case 0x7e:  // movq
                    ip = &db_inst_special[SPEC_F30F7E_MOVQ];
                    break;
                case 0x7f:  // movdqu
                    ip = &db_inst_special[SPEC_F30F7F_MOVDQU];
                    break;
            }
            if (ip != NULL)
                *loc += 2;
...
#endif
{
...
#ifdef PORT_MED
        int realmode = altfmt & 2;
        if (realmode) {
            short_addr = TRUE;
            size       = WORD;
        }

        db_printf("%8x:  ", (uint32_t)loc);
#endif
	...
	for (first = TRUE;
		...
		switch (i_mode & 0xFF) {
			...
			break;
                    case MX:
                        db_printf("%%mm%d", f_reg(regmodrm));
                        break;
                    case EMX:
                        if (f_mod(regmodrm) == 3)
                            db_printf("%%mm%d", f_rm(regmodrm));
                        else
                            db_print_address(seg, size, &address);
                        break;
                    case XMM:
                        db_printf("%%xmm%d", f_reg(regmodrm));
                        break;
                    case EXMM:
                        if (f_mod(regmodrm) == 3)
                            db_printf("%%xmm%d", f_rm(regmodrm));
                        else
                            db_print_address(seg, size, &address);
                        break;
		    case Rw:
