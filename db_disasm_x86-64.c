...
#ifndef PORT_MED
static const char * const db_index_reg_32[8] = {
	"%ebx,%esi",
	"%ebx,%edi",
	"%ebp,%esi",
	"%ebp,%edi",
...
static const char * const db_reg[2][4][8] = {
{
...
static const char * const rex_str[0x10] = {
	"rex      ",    /* 0x40 */
	"rex.b    ",    /* 0x41 */
	"rex.x    ",    /* 0x42 */
	"rex.xb   ",    /* 0x43 */
...
void db_format_radix(char *buf, size_t bufsiz, uint32_t val, int altflag);
...
static db_addr_t db_read_address(db_addr_t, u_int, int, int, struct i_addr *);
static void db_print_address(const char *, u_int, int, struct i_addr *);
static db_addr_t db_disasm_esc(db_addr_t, int, u_int, int, int, const char *);
...
{
	unsigned int	mod, rm, sib, index, disp, ext;
...
#ifdef PORT_MED
db_addr_t
db_disasm_64(db_addr_t loc, boolean_t altfmt)
#else
db_addr_t
